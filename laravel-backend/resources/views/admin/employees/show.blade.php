...
<div class="space-y-6">
        ...
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-sm text-slate-500 mt-0.5">{{ $employee->phone }} Â· {{ $employee->jabatan ?? '-' }}</p>
                    <div class="flex items-center gap-2 mt-2">
                        @if($employee->face_embedding)
                            <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-emerald-100 text-emerald-700 rounded-full">
                                <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"></path></svg>
                                Wajah Terdaftar
                            </span>
                        @else
                            <span class="inline-flex px-2.5 py-1 rounded-lg text-xs font-medium bg-gray-100 text-slate-500">Wajah Belum Terdaftar</span>
                        @endif
                    </div>
                </div>
            </div>
    ...
    <div class="grid grid-cols-3 gap-4">
        <div class="stat-card text-center">
            ...
            <p class="text-xs text-slate-500 mt-1 font-medium">Hadir</p>
        </div>
        <div class="stat-card text-center">
            <p class="text-3xl font-extrabold text-red-500">{{ $totalTerlat }}</p>
            <p class="text-xs text-slate-500 mt-1 font-medium">Terlambat</p>
        </div>
        <div class="stat-card text-center">
            <p class="text-3xl font-extrabold text-purple-600">{{ $totalLembur }}</p>
            <p class="text-xs text-slate-500 mt-1 font-medium">Lembur</p>
        </div>
    </div>
    ...
    <div class="table-container">
        <div class="px-6 py-5 border-b border-slate-100/80 flex items-center justify-between">
            <h3 class="text-sm font-bold text-slate-900">History â€” {{ \Carbon\Carbon::parse($month)->translatedFormat('F Y') }}</h3>
                ...
                <tbody class="divide-y divide-slate-100/80">
                    @forelse($attendances as $att)
                        <tr class="hover:bg-gray-50/50">
                            <td class="px-6 py-3 text-slate-600">{{ \Carbon\Carbon::parse($att->timestamp)->format('d M Y') }}</td>
                            <td class="px-6 py-3"><span class="px-2 py-0.5 rounded-full text-xs font-bold {{ $att->type === 'Masuk' ? 'bg-emerald-100 text-emerald-700 rounded-full' : 'bg-blue-100 text-blue-700 rounded-full' }}">{{ $att->type }}</span></td>
                            <td class="px-6 py-3 font-mono text-slate-900 font-semibold">{{ \Carbon\Carbon::parse($att->timestamp)->format('H:i') }}</td>
                            <td class="px-6 py-3">
                            ...
                            <td class="px-6 py-3 text-slate-400 text-xs">{{ $att->location_name ?? '-' }}</td>
                        </tr>
                    @empty
                        <tr><td colspan="5" class="px-6 py-12 text-center text-slate-400 text-sm">Tidak ada data absensi bulan ini</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    ...
    <a href="{{ route('admin.employees.index') }}" class="inline-flex items-center gap-2 text-sm text-slate-500 hover:text-slate-700">
